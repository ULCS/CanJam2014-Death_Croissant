...
public class moleAnimator : MonoBehaviour {
	...
	void Start () {
		...
		dust = this.gameObject.transform.parent.transform.FindChild("Dust").
			gameObject.GetComponent<ParticleSystem>();
		trackableHandler = GameObject.Find("ImageTarget").gameObject.GetComponent<MoleTrackableEventHandler>();
		
		iTween.MoveBy(gameObject, iTween.Hash("y", 40, "easeType", "easeInOutQuad","speed",20, 
			"delay", Random.Range(0.3f,5f), "oncomplete","animComplete_Up"));
	}
	
	//Function called when animation below ground is complete
	public void animComplete_Down()
	{
		//Only make the mole character visible if the Tackable is visible. This is to avoid having the
	...
	{
		iTween.MoveBy(gameObject, iTween.Hash("y", -40, "easeType", "easeInOutQuad","speed",20, 
			"delay",Random.Range(0.3f,1f), "oncomplete","animComplete_Down"));
	}
