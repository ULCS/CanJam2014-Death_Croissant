...
namespace tk2dEditor.SpriteCollectionEditor
{
	public class TextureEditor
	{
		...
		tk2dSpriteCollectionDefinition.ColliderData currentInspectedData = null;

		tk2dSpriteCollectionDefinition.ColliderData SelectedColliderData(tk2dSpriteCollectionDefinition param) {
		...
		void DrawAdvancedColliderInspector(tk2dSpriteCollectionDefinition param) {
			...
			if (param.colliderData.Count == 0) {
				tk2dGuiUtility.InfoBox("Advanced Colliders" +
						  "\nDouble click in the texture view to create a collider" +
						  "\nSelect the collider and Edit > Delete to delete"
						  ,tk2dGuiUtility.WarningLevel.Info);
			}

			tk2dSpriteCollectionDefinition.ColliderData selection = SelectedColliderData( param );
			if (currentInspectedData != selection) {
				if (Event.current.type != EventType.Layout) HandleUtility.Repaint();
				else currentInspectedData = selection;
			}
			if (currentInspectedData != null) {
				currentInspectedData.type = (tk2dSpriteCollectionDefinition.ColliderData.Type)EditorGUILayout.EnumPopup("Type", currentInspectedData.type);
				currentInspectedData.origin = EditorGUILayout.Vector2Field("Origin", currentInspectedData.origin);
				if (currentInspectedData.type == tk2dSpriteCollectionDefinition.ColliderData.Type.Circle) {
					float x = EditorGUILayout.FloatField("Radius", currentInspectedData.size.x);
					currentInspectedData.size.Set(x, x);
				}
				else {
					currentInspectedData.size = EditorGUILayout.Vector2Field("Size", currentInspectedData.size);
					currentInspectedData.angle = EditorGUILayout.FloatField("Angle", currentInspectedData.angle);
				}
			}
		}
		...
		void DrawAdvancedColliderEditor(Rect r, tk2dSpriteCollectionDefinition param, Texture2D tex) {
			...
			if (ev.type == EventType.MouseDown && ev.clickCount == 2) {
				...
				d.origin = (ev.mousePosition - origin) / editorDisplayScale;
				float m = Mathf.Min(tex.width, tex.height) / 4;
				d.size = new Vector2(m, m);
				d.angle = 0;
				param.colliderData.Add(d);
				HandleUtility.Repaint();
				tk2dGuiUtility.ActiveTweakable = advancedColliderEditorControlBase + param.colliderData.Count - 1;
				ev.Use();
			}
		}
		...
		public void DrawTextureInspector(tk2dSpriteCollectionDefinition param, Texture2D texture)
		{
			if (mode == Mode.Collider && param.colliderType == tk2dSpriteCollectionDefinition.ColliderType.Polygon)
			{
				...
				DrawAdvancedColliderInspector(param);
			}
			if (mode == Mode.Texture && param.customSpriteGeometry)
			{
				param.colliderColor = (tk2dSpriteCollectionDefinition.ColliderColor)EditorGUILayout.EnumPopup("Display Color", param.colliderColor);
